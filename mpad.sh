...
sudo apt-get update -y --allow-releaseinfo-change | pv -t
sudo apt-get upgrade -y | pv -t

# Step 7: Raspi-config
echo "Please follow the instructions in Step 7 to update the Raspberry Pi settings manually." | pv -t
...
mkdir projects
cd projects
git clone https://github.com/ishan190425/DonkeyCustom | pv -t
cd DonkeyCustom
git checkout main
pip install -y -e .[pi] | pv -t
pip install -y -r mpad.txt | pv -t
pip install -y https://github.com/lhelontra/tensorflow-on-arm/releases/download/v2.2.0/tensorflow-2.2.0-cp37-none-linux_armv7l.whl | pv -t
